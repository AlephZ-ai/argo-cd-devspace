...
while ! (kind create cluster --wait "$timeout" --config "$KINDEST_ARGO_CD_PROJECT_ROOT/src/clusters/$KINDEST_ARGO_CD_CLUSTER_NAME.yaml" 2>/dev/null); do kind delete clusters "$KINDEST_ARGO_CD_CLUSTER_NAME"; done
