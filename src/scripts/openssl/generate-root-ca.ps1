...
openssl req -x509 -nodes -new -sha256 -days 3650 -newkey rsa:2048 `
    ...
    -out "$env:KINDEST_ARGO_CD_PROJECT_ROOT/devcerts/$certpurpose/cert.pem" -subj $subject
openssl x509 -outform pem -in "$env:KINDEST_ARGO_CD_PROJECT_ROOT/devcerts/$certpurpose/cert.pem" `
